...
const TagCard = ({ number, title, text, meta, stack = [], className, aosDelay, aosType, pathLength, containerRef }) => {
  ...
  return (
    <div 
      ...
      className={`w-72 sm:w-80 rounded-[2rem] p-2 relative flex flex-col items-center hover:scale-[1.02] transition-all duration-700 z-10 ${className} ${
        isActive ? 'bg-[#0033A0] border-blue-400 shadow-[0_20px_50px_rgba(0,51,160,0.4)]' : 'bg-white border border-gray-200 shadow-[0_15px_40px_rgba(0,0,0,0.06)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.12)]'
      }`}
    >
      {/* The hole punch */}
      ...
      <div className={`w-full h-full rounded-[1.5rem] mt-8 p-8 flex flex-col min-h-[260px] transition-colors duration-700 ${
        isActive ? 'bg-blue-900/50' : 'bg-[#f4f4f4]'
      }`}>
        ...
        {meta && (
          <span className={`text-[11px] font-bold uppercase tracking-wider mb-3 transition-colors duration-700 ${
            isActive ? 'text-blue-200' : 'text-gray-400'
          }`}>{meta}</span>
        )}
        ...
        {stack.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-4">
            {stack.map((tech) => (
              <span
                key={tech}
                className={`text-[10px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-full transition-colors duration-700 ${
      ...
      <div className="max-w-6xl mx-auto relative md:h-[1350px]">
        ...
        <div data-aos="fade-up" className="md:absolute top-10 left-0 md:w-[450px] z-20 mb-16 md:mb-0">
          ...
          <p className="text-gray-500 text-base md:text-lg max-w-sm font-medium leading-relaxed">
            A selection of academic and hackathon projects I have built across the MERN stack, deep learning, and full-stack web development.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-8 md:gap-12 items-center md:block relative z-10 w-full pt-4 md:pt-0 pb-12 md:pb-0">
          ...
          <TagCard
            number="01"
            title="Skill Training Schedule Generation System"
            meta="Dec 2024 – Apr 2025 · Frontend Developer"
            text="A centralized, automated system for managing academic training workflows — streamlining budget management, attendance tracking, and feedback collection for real-time data access and better decision-making."
          ...
          <TagCard
            number="02"
            title="Online Discourse Platform"
            meta="Oct 2023 – Dec 2023 · Frontend Developer"
            text="A web-based platform that links users together so they can connect and clarify technical doubts from anywhere, backed by a user database holding complete member details."
            stack={["React JS", "MongoDB", "VS Code"]}
          ...
          <TagCard
            ...
            meta="Jul 2023 – Aug 2023 · Developer"
            text="An online music streaming platform designed so that users can easily access and stream music over the internet from any device."
            stack={["React JS", "MongoDB", "VS Code"]}
          ...
          <TagCard
            number="04"
            title="Heritage Classification of Monuments"
            meta="Dec 2022 – Feb 2023 · Deep Learning · 🏆 1st Place, BIT'S Hack 2022"
            text="Used deep learning to classify architectural heritage images and monuments into their respective labels with the highest possible accuracy. Won 1st place (₹10,000) at BIT'S Hack 2022."
            stack={["Python", "Deep Learning", "Colab"]}
